...
export function TransportManagementScreen({ eventId }: { eventId: string }) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiClient.delete(endpoints.transportAdmin.lines.delete(eventId, pendingDeleteLineId));
      if (selectedLineId === pendingDeleteLineId) {
        setSelectedLineId(null);
        setSelectedStops([]);
        setSelectedSchedules([]);
      }
  ...
  const abrirModalStops = () => {
    setStopsDraft(selectedStops.map((s) => ({ zone_id: s.zone_id, stop_order: s.stop_order })));
    setStopsError(null);
  ...
  return (
    <div className="space-y-10">
        ...
        {linesLoading ? (
          ...
          <div className="bg-white rounded-lg border border-slate-200 divide-y divide-slate-100">
            {lines.map((line) => (
              <div
                key={line.id}
                className={`px-4 py-3 ${selectedLineId === line.id ? 'bg-indigo-50' : ''}`}
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setPendingDeleteLineId(line.id)}
                      title="Eliminar"
                  ...
                  <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <Button variant="secondary" size="sm" onClick={abrirModalStops}>
                          Editar Paradas
                        </Button>
                      </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Button variant="primary" size="sm" onClick={abrirModalHorarios}>
                            Editar Horarios
                          </Button>
                        </div>
                      </div>
      ...
      {/* Modal línea */}
      {lineModal && (
        <ModalFrame title={lineModal.mode === 'create' ? 'Nueva Línea' : 'Editar Línea'}>
          <div>
      ...
      {stopsModalOpen && selectedLine && (
        <ModalFrame title={`Paradas de ${selectedLine.name}`}>
      ...
      {scheduleModalOpen && selectedLine && (
        <ModalFrame title={`Horarios de ${selectedLine.name}`}>
          {selectedStops.length === 0 ? (
            <p className="text-sm text-slate-500 italic">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              ...
              <Button variant="ghost" size="sm" onClick={agregarHorario} className="text-indigo-600 hover:bg-indigo-50">
                <Plus className="w-4 h-4" />
                Agregar horario
              </Button>
            </div>
      ...
      <ConfirmDialog
        open={pendingDeleteLineId !== null}
        title="Eliminar línea"
        message={`¿Eliminar la línea "${lines.find((l) => l.id === pendingDeleteLineId)?.name ?? ''}"? También se eliminarán sus paradas y horarios. Esta acción no se puede deshacer.`}
        confirmLabel="Eliminar"
        cancelLabel="Cancelar"
        variant="destructive"
        onConfirm={() => void handleDeleteConfirm()}
        onCancel={() => setPendingDeleteLineId(null)}
      />
    </div>
  );
}
...
function ModalFrame({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50">
      <Card variant="standard" className="w-full max-w-4xl mx-4 space-y-4">
