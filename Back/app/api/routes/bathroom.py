...
from src.interfaces.rest.bathroom_product import get_bathroom_product_adapter

router = APIRouter(prefix="/api/events/{event_id}", tags=["Bathroom Product"])
...
async def bathroom_recommendations(
    ...
    result = await get_bathroom_product_adapter(
        db=db,
        timestamp=now,
        event_id=event_id,
