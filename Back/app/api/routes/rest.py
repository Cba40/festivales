...
router = APIRouter(prefix="/api/events/{event_id}", tags=["Rest Product"])


@router.get("/products/rest", response_model=RestRecommendationResponse)
async def rest_recommendations(
    event_id: str,
    ...
    try:
        result = await get_rest_product_adapter(
            db=db,
            timestamp=now,
            event_id=event_id,
