...
class OperationalPhaseModel(Base):
    ...
    operational_profile_id: Mapped[UUID] = mapped_column(ForeignKey("operational_profiles.id", ondelete="CASCADE"), nullable=False)
    name: Mapped[str] = mapped_column(String(100), nullable=False)
    sort_order: Mapped[int] = mapped_column(nullable=False)

    __table_args__ = (
        UniqueConstraint("operational_profile_id", "sort_order", name="uq_phase_profile_sort_order"),
        CheckConstraint("sort_order >= 0", name="ck_operational_phases_sort_order_ge_0"),
    )
