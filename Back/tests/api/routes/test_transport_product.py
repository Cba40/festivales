"""Tests for the GET /api/events/{event_id}/products/transport endpoint.
...
class TestTransportProductEndpoint:
    def test_200_ok(
        self,
        client: TestClient,
        ...
        assert first["name"] == "Terminal Principal"
        ...
        assert first["calle"] == "Av. San Martín"
        assert first["lat"] == -30.97
        assert first["lng"] == -64.08
        assert first["referencia"] == "Frente a la Plaza"
