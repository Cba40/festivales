...
from app.schemas.product import RestRecommendationResponse, ZonaRestItem
...
def _mock_adapter(
    ...
    with patch(
        "app.api.routes.rest.get_rest_product_adapter",
        new_callable=AsyncMock,
    ) as mock:
        mock.return_value = RestRecommendationResponse(
            event_id=EVENT_ID,
...
class TestRestProductEndpoint:
    def test_200_ok(
        ...
        assert first["name"] == "Descanso Norte"
        ...
        assert first["referencia"] == "Sector de descanso"
    ...
    def test_empty_zones(
        ...
        _mock_adapter.return_value = RestRecommendationResponse(
            event_id=EVENT_ID,
