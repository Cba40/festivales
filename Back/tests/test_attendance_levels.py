...
class TestAttendanceLevelCRUD:
    ...
    def test_create_valid_ranges(self, db_session: Session, sample_event: Event):
        ...
        crud.create(db_session, AttendanceLevelCreate(name="Media", min_people=5001, max_people=15000), sample_event.id)
        crud.create(db_session, AttendanceLevelCreate(name="Alta", min_people=15001, max_people=30000), sample_event.id)
        crud.create(db_session, AttendanceLevelCreate(name="Muy Alta", min_people=30001, max_people=60000), sample_event.id)
        crud.create(db_session, AttendanceLevelCreate(name="Masiva", min_people=60001, max_people=None), sample_event.id)
    ...
    def test_create_overlapping_ranges_raises_422(self, db_session: Session, sample_event: Event):
        ...
        with pytest.raises(Exception) as exc:
            crud.create(db_session, AttendanceLevelCreate(name="Solapada", min_people=2500, max_people=8000), sample_event.id)
    ...
    def test_create_ranges_with_gaps_raises_422(self, db_session: Session, sample_event: Event):
        crud.create(db_session, AttendanceLevelCreate(name="Baja", min_people=0, max_people=5000), sample_event.id)
        with pytest.raises(Exception) as exc:
            crud.create(db_session, AttendanceLevelCreate(name="ConHueco", min_people=7000, max_people=15000), sample_event.id)
    ...
    def test_create_multiple_null_max_people_raises_422(self, db_session: Session, sample_event: Event):
        crud.create(db_session, AttendanceLevelCreate(name="Baja", min_people=0, max_people=5000), sample_event.id)
        crud.create(db_session, AttendanceLevelCreate(name="Masiva1", min_people=5001, max_people=None), sample_event.id)
        with pytest.raises(Exception) as exc:
            crud.create(db_session, AttendanceLevelCreate(name="Masiva2", min_people=5001, max_people=None), sample_event.id)
        assert "422" in str(exc.value)

    def test_update_range_validation(self, db_session: Session, sample_event: Event):
        crud.create(db_session, AttendanceLevelCreate(name="Baja", min_people=0, max_people=5000), sample_event.id)
        media = crud.create(db_session, AttendanceLevelCreate(name="Media", min_people=5001, max_people=15000), sample_event.id)
        crud.create(db_session, AttendanceLevelCreate(name="Masiva", min_people=15001, max_people=None), sample_event.id)
